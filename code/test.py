...
def display_data_gui(df, stl_type, language_names, languages_to_parse=['English'], current_file_path=None):
    ...
    if current_file_path:
        ...
        root.title(f"STL Data Editor - {file_name}")
    else:
    ...
    for col in columns_to_display:
        ...
        if col in ['string_id', 'id']:
            ...
            tree.column(col, anchor='w', width=400)
    ...
    def update_treeview(reset=False):
        ...
        if reset or not search_var.get():
            display_df = df
            search_var.set('')
        else:
            search_text = search_var.get().lower()
            display_df = df[df.apply(lambda row: row.astype(str).str.contains(search_text, case=False).any(), axis=1)]
...
if __name__ == "__main__":
    ...
    # Display data using the GUI
    display_data_gui(df, stl_type, language_names, languages_to_parse, file_path)